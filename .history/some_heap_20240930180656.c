...
void heap_bubble_up(heap_t *heap, int index) {
    ...
    heap_node_t leftChild = heap->data[heap_left_child[index]];
     while(curr_heap_level != 0){
        if (heap_left_child(index) < heap_right_child(index)){
            if (heap_left_child(index) < heap_parent(index)){
                heap_swap(heap_left_child(index), heap_parent(index));
            }
